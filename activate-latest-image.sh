...
# Activate Latest Image Script for Legal Document RAG System
...
echo "🔄 Activating latest image for Legal Document RAG System..."
...
CURRENT_TASK_DEF=$(aws ecs describe-task-definition --task-definition $TASK_DEFINITION_NAME --region $AWS_REGION --output json)

if [ $? -ne 0 ]; then
    echo "❌ Failed to get current task definition. Please ensure the service exists."
    exit 1
fi

echo "✅ Found current task definition"
...
NEW_REVISION=$(aws ecs register-task-definition --cli-input-json file://task-definition-update.json --region $AWS_REGION --query 'taskDefinition.revision' --output text)
...
echo ""
echo "🎉 Latest image activation completed!"
echo ""
...
if [ "$ALB_DNS" != "Load balancer not found" ]; then
    ...
    echo ""
    echo "💡 Test the application to ensure it's working correctly."
else
